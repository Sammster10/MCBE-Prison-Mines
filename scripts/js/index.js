...
export function setRefillStallMS(ms) {
    refillStallMS = ms;
}
world.events.worldInitialize.subscribe((e) => {
    ...
    Object.keys(obj).forEach((key) => {
        const mine = obj[key];
        const m = new PrisonMine(mine.name, mine.min, mine.max, mine.blocks, false, mine.currentBlocks, mine.maxRefillTimer, mine.refillPercent, mine.teleportLoc);
        let blocks = new WeightedElements();
        Object.keys(mine.blocks.elements).forEach((key) => {
            blocks.add(mine.blocks.elements[key].element, mine.blocks.elements[key].weight);
        });
        m.setBlocks(blocks);
...
world.events.blockBreak.subscribe((e) => {
    ...
    const mine = Object.values(PrisonMine.data).find((mine) => {
        return (location.x >= mine.min.x &&
    ...
    if (!mine)
        return;
    mine.currentBlocks--;
    if (mine.currentBlocks <= (mine.maxBlocks * mine.refillPercent) / 100 &&
        !mine.isRefilling) {
        mine.refill();
    }
});
export let refillTasks = [];
