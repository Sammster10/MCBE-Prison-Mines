...
function createMine(player: Player) {
  let form = new ModalFormData()
    .title("Create New Mine")
    .textField("Mine Name", "Mine Name")
    .textField(
      "Mine Coordinates (x1, y1, z1, x2, y2, z2)",
      "X1, Y1, Z1, X2, Y2, Z2"
    );
  ...
  form.show(player).then((response) => {
    ...
    const mineCoords = mineCoordsStrs.map((str: string) => parseInt(str));
